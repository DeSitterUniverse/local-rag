...
export default function App() {
  ...
  const [isModelMenuOpen, setIsModelMenuOpen] = useState(false); // NEW STATE
  ...
  // --- 2. Bulletproof Drag & Drop ---
  useEffect(() => {
    // We store the PROMISES instead of the resolved functions
    const unlistenV1Promise = listen("tauri://file-drop", (event: any) => {
      const paths = event.payload as string[];
      if (paths && paths.length > 0) handleIngest(paths[0]);
    });
    
    const unlistenV2Promise = listen("tauri://drag-drop", (event: any) => {
      if (event.payload?.paths && event.payload.paths.length > 0) {
        handleIngest(event.payload.paths[0]);
      }
    });
    ...
    return () => {
      // When React unmounts, wait for the promises to finish, THEN destroy them
  ...
  async function handleIngest(filePath: string) {
    setIsSidebarOpen(true); // Pop sidebar open so they can see the progress
    setIngestStatus({ message: "Processing reference...", type: 'info' });
    try {
      ...
      if (data.status === "success") {
        setIngestStatus({ message: `Success! Added to knowledge base.`, type: 'success' });
        fetchDocuments();
      } else {
  ...
  if (!backendReady) return <div style={{ display: 'flex', height: '100vh', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>Loading Cephalon...</div>;
  ...
  return (
    <div style={{ display: "flex", height: "100vh", fontFamily: "system-ui, sans-serif", position: "relative", backgroundColor: "#f9fafb" }}>
      ...
      {/* COLLAPSIBLE SIDEBAR: File Reference Library */}
      <div style={{ 
        ...
        <div style={{ padding: "1.5rem", borderBottom: "1px solid #e5e7eb", minWidth: "320px", boxSizing: "border-box" }}>
          ...
          <input 
            ...
            style={{ width: "100%", padding: "0.5rem 0.75rem", borderRadius: "6px", border: "1px solid #d1d5db", boxSizing: "border-box", fontSize: "0.875rem" }}
          />
        </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                    {docs.map(doc => (
                      <div key={doc.id} style={{ backgroundColor: "#f9fafb", padding: "0.75rem", borderRadius: "8px", border: "1px solid #f3f4f6", fontSize: "0.875rem" }}>
                        <div style={{ fontWeight: 500, color: "#374151", wordBreak: "break-all", marginBottom: "0.5rem" }} title={doc.name}>
                          {doc.name}
                        </div>
        ...
        <div style={{ padding: "1rem 1.5rem", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center", backgroundColor: "white", zIndex: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
            <button 
              ...
              style={{ background: "none", border: "1px solid #e5e7eb", borderRadius: "6px", padding: "0.4rem 0.6rem", cursor: "pointer", color: "#4b5563", backgroundColor: isSidebarOpen ? "#f3f4f6" : "white" }}
            >
              {isSidebarOpen ? "◀" : "▶"}
            </button>
            <h1 style={{ margin: 0, fontSize: "1.25rem", color: "#111827", fontWeight: 700, letterSpacing: "-0.5px" }}>CephalonMemory</h1>
          </div>
          ...
          <div style={{ position: "relative" }}>
            ...
            <button 
              ...
              style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 1rem", borderRadius: "9999px", border: "1px solid #e5e7eb", backgroundColor: isModelMenuOpen ? "#f3f4f6" : "white", fontSize: "0.875rem", color: "#374151", cursor: "pointer", fontWeight: 500, boxShadow: "0 1px 2px rgba(0,0,0,0.05)", transition: "all 0.2s" }}
            >
              <span style={{ color: "#8b5cf6" }}>✨</span>
              {selectedModel || "Loading engine..."}
              <span style={{ fontSize: "0.7rem", color: "#9ca3af", transform: isModelMenuOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}>▼</span>
            </button>
            ...
            {isModelMenuOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 0.5rem)", right: 0, width: "240px", backgroundColor: "white", border: "1px solid #e5e7eb", borderRadius: "12px", boxShadow: "0 10px 15px -3px rgba(0,0,0,0.1), 0 4px 6px -2px rgba(0,0,0,0.05)", zIndex: 40, overflow: "hidden", display: "flex", flexDirection: "column" }}>
                <div style={{ padding: "0.75rem 1rem", backgroundColor: "#f9fafb", borderBottom: "1px solid #e5e7eb", fontSize: "0.75rem", fontWeight: 600, color: "#6b7280", letterSpacing: "0.05em", textTransform: "uppercase" }}>
                  Active Neural Engine
                </div>
                <div style={{ maxHeight: "300px", overflowY: "auto" }}>
                  {models.length === 0 ? (
                    <div style={{ padding: "1rem", fontSize: "0.875rem", color: "#9ca3af", textAlign: "center" }}>No models found.</div>
                  ) : (
                    models.map(m => (
                      <button
                        key={m}
                        onClick={() => { setSelectedModel(m); setIsModelMenuOpen(false); }}
                        style={{ width: "100%", textAlign: "left", padding: "0.875rem 1rem", backgroundColor: selectedModel === m ? "#f5f3ff" : "white", border: "none", borderBottom: "1px solid #f3f4f6", fontSize: "0.875rem", color: selectedModel === m ? "#6d28d9" : "#374151", cursor: "pointer", fontWeight: selectedModel === m ? 600 : 400, transition: "background-color 0.1s", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                      >
                        {m}
                        {selectedModel === m && <span style={{ color: "#8b5cf6" }}>✓</span>}
                      </button>
                    ))
                  )}
                </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "2rem", display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          {messages.length === 0 && (
            <div style={{ margin: "auto", textAlign: "center", color: "#6b7280", maxWidth: "400px" }}>
              ...
              <p style={{ lineHeight: "1.5" }}>I remember our past conversations. You can chat normally, or drop files into this window to add them to my reference library.</p>
            </div>
          )}
          {messages.map((msg, i) => (
          ...
          {isTyping && messages[messages.length - 1]?.role === "user" && (
            <div style={{ display: "flex", justifyContent: "flex-start" }}>
               <div style={{ backgroundColor: "#f3f4f6", padding: "1rem 1.25rem", borderRadius: "16px", color: "#6b7280", fontSize: "0.875rem", fontStyle: "italic" }}>
        ...
        <div style={{ padding: "1.5rem", borderTop: "1px solid #e5e7eb", backgroundColor: "white" }}>
          <form onSubmit={handleSend} style={{ display: "flex", gap: "0.75rem", maxWidth: "56rem", margin: "0 auto" }}>
            <input 
              ...
              placeholder="Record a thought, or ask me anything..." 
              disabled={isTyping} 
              style={{ flex: 1, padding: "1rem 1.25rem", borderRadius: "12px", border: "1px solid #d1d5db", fontSize: "1rem", outline: "none", boxShadow: "inset 0 2px 4px rgba(0,0,0,0.02)" }} 
            />
            <button 
              type="submit" 
              disabled={isTyping || !input.trim()} 
              style={{ padding: "0 1.5rem", borderRadius: "12px", fontWeight: 600, cursor: (isTyping || !input.trim()) ? "not-allowed" : "pointer", backgroundColor: (isTyping || !input.trim()) ? "#e5e7eb" : "#111827", color: (isTyping || !input.trim()) ? "#9ca3af" : "white", border: "none", transition: "all 0.2s" }}
            >
              Send
            </button>
          </form>
